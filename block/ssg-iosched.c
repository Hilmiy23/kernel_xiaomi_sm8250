...
/*
 * This kernel has no blk_discard_mergable(); this is what blk_try_merge()
 * here uses to decide that a discard request can absorb more bios.
 */
static inline bool ssg_discard_mergable(struct request *req)
{
...
/*
 * The .completed_request hook here takes no timestamp, so sample one the
 * same way kyber-iosched does.
 */
static void ssg_completed_request(struct request *rq)
{
	...
	rqi = ssg_rq_info(ssg, rq);
	if (likely(rqi)) {
		ssg_stat_account_io_done(ssg, rq, rqi->data_size,
				ktime_get_ns());
	}
}
...
static void ssg_prepare_request(struct request *rq, struct bio *bio)
{
	...
	if (likely(rqi)) {
		...
		rcu_read_lock();
		rqi->blkg = blkg_lookup(ssg_current_blkcg(), rq->q);
